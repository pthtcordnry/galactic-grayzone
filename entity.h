...
static char *EntityAssetToJSON(const EntityAsset *asset)
{
    ...
    strncat(json, ",\n", bufSize - strlen(json) - 1);
    AppendAnimationFrames(&json, &bufSize, "jump", &asset->jump);
    strncat(json, ",\n", bufSize - strlen(json) - 1);
...
static bool EntityAssetFromJSON(const char *json, EntityAsset *asset)
{
    if (!json || !asset)
        return false;
    ...
    // Load the texture if a texture path was provided.
    ...
    ParseAnimation(json, "jump", &asset->jump);
